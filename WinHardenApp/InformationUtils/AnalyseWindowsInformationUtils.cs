...
namespace WinHardenApp.AnalyzeInformationUtils
{
    internal class AnalyseWindowsInformationUtils
    {
        ...
        internal void AnalyseWindowsInformation()
        {
            ...
            if (File.Exists(m_configurationFilesFolder + @"\log.txt"))
            {
                LogUtils.AddLogEntry("There were errors during extraction security configuration files process. We include the content from log.txt file."+Environment.NewLine);
                string logContent = File.ReadAllText(m_configurationFilesFolder + @"\log.txt");
                LogUtils.AddLogEntry(logContent);
            }

            m_windowsInformationUtils.UpdateProgressBar("Copying hostname file");
            if (File.Exists(m_configurationFilesFolder + @"\hostname.txt"))
            {
                File.Copy(m_configurationFilesFolder + @"\hostname.txt", m_analysisOutputFolder + @"\hostname.txt", true);
            }
            else
            {
                LogUtils.AddLogEntry("Hostname.txt file is not found in folder with extracted files.");
            }
   
            //This method changes the regional information for the current subprocess.
